...
任务类型包括：
- tool_execution: 需要调用外部工具的任务（如查询/创建考勤、通知等OA操作）
...
重要约束：
- 对于纯查询类请求（如"查考勤"、"看通知"、"查记录"），一次 tool_execution 即可完成
- **不要创建** "格式化结果"、"整理数据"、"返回给用户" 等纯展示性质的子任务——展示是系统自动完成的
- 避免过度分解：一个工具调用能解决的问题，就不要拆成多个子任务
- 只有确实需要不同工具链时（如"先查用户信息，再查该用户的考勤"）才分解为多个子任务

请按照以下要求进行分解：
1. 分析用户意图，确定主任务类型
2. 如果需要多个工具协作，再分解为子任务
3. 定义子任务的优先级和依赖关系
4. 明确每个子任务需要的参数
